...
function transformToLis(obj){
  ...
  Object.entries(obj).forEach(value=>{
    array.push(`<li>${value[0]}: ${value[1]}</li>`)
...
const count = (target, input) => {
  // Solution code here...
  let count = input.reduce((instance, value, idx) => {

    instance+=value.reduce((instance2, value2, index2) => {
      if (value2 == target) {
        instance2 += 1;
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.map((value,index)=>{
    ...
    value.map((value2,index2)=>{
      // console.log(value2,'type of value 2 ',typeof value2,'is value===number',value2==='number');
      // console.log(typeof typeof 5);

      if( typeof value2==='number' && value2%5===0){
        subarray.push(Math.pow(2, value2));
      }
    })
    array.push(subarray);
...
let findShortest = (data) => {
  ...
  let array = [];
  data.map(value => {
    if (Number(value.height) < testH) { testH = Number(value.height) }
  })
  let x=data.filter(value=>Number(value.height)===testH);
  return x[0].name
