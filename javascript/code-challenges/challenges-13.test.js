...
// Solution code here...
let testLength=0;
let testindex= arr.reduce((acc,value,index)=>{

  // console.log(value,index,value.length,testLength,acc);
...
if(   value.includes(':)')){array.push(value)}
})
...
const standardizePhoneNumbers = (arr) => {
  ...
  arr.map((value,index)=>{
    ...
    // console.log(value.substring(6,9));
    // console.log(value.substring(11-15));
   array.push(value.substring(1,4)+value.substring(6,9)+value.substring(10,14))
...
const onlyOddChars = (str) => {
  // Solution code here...
  let array=[];
  let x =str.split('').reduce((acc,value,index)=>{
    if(index%2===1){
      acc+=value;
      console.log(value,index,acc);
